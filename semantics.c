    ...
    do {
        ...
        switch (h->op) {
            case ASSIGNOP:  printf("  %s := %s\n",resultstr,opnstr1);
                            break;
            case ArrRef:    printf("  *(%s) ", resultstr);
                            break;
            case PLUS:
            case MINUS:
            case STAR:
...
void semantic_error(int line,char *msg1,char *msg2){
    //这里可以只收集错误信息，最后在一次显示
    printf("\e[31m在%d行,%s %s\e[0m\n",line,msg1,msg2);
}
void prn_symbol(){ //显示符号表
    int i=0;
    printf("%6s    %6s   %6s  %6s %4s %6s\n","变量名","别 名","层 号","类型","标记","偏移量");
    for(i=0;i<symbolTable.index;i++)
        printf("%6s %6s %6d  %6s %4c %6d\n",symbolTable.symbols[i].name,\
                symbolTable.symbols[i].alias,symbolTable.symbols[i].level,\
                symbolTable.symbols[i].type.kind==INT?"int":(symbolTable.symbols[i].type.kind == FLOAT ? "float" : "char"),\
...
int fillSymbolTable(char *name,char *alias,int level,int type,char flag,int offset) {
    ...
    for(i=symbolTable.index-1;(level==0 && i>=0)||(i>=0&&symbolTable.symbols[i].level==level);i--) {
    ...
    symbolTable.symbols[symbolTable.index].type.kind=type;
...
void ext_var_list(struct node *T){  //处理变量列表
    ...
    switch (T->kind){
        ...
        case ArrDef:
            ...
            T->ptr[0]->type=T->type;              //将类型属性向下传递变量结点
            T->ptr[0]->offset=T->offset;          //外部变量的偏移量向下传递
            T->ptr[0]->dimension = 1;       // Initialize array dimension
            ext_var_list(T->ptr[0]);
            T->num = T->ptr[0]->num;
        ...
        case ArrDec:        //TODO
            ...
            T->ptr[0]->type=T->type;              //将类型属性向下传递变量结点
            T->ptr[0]->offset=T->offset;          //外部变量的偏移量向下传递
            T->ptr[0]->dimension = T->dimension;
            if (T->ptr[0]->kind == ArrDec){
                T->ptr[0]->dimension++;
                ext_var_list(T->ptr[0]);
                T->num = T->ptr[0]->num;
                ...
                rtn=fillSymbolTable(T->ptr[0]->type_id,newAlias(),LEV,T->ptr[0]->type,'V',T->ptr[0]->offset);  //最后一个变量名
                if (rtn==-1)
                    semantic_error(T->pos,T->type_id, "变量重复定义");
                else {
                    T->place=rtn;
                    symbolTable.symbols[rtn].dimension = T->dimension;
                    //printf("Array Dimension is %d.\n", T->dimension);  //DEBUG TODO
                    symbolTable.symbols[rtn].type.kind = ARRAY;
                    symbolTable.symbols[rtn].type.arryKind = T->type;
                }
            }

            symbolTable.symbols[T->place].type.size[symbolTable.symbols[T->place].dimension - T->dimension] = T->type_int;
...
int  match_param(int i,struct node *T){
    ...
    for (j=1;j<num;j++) {
        ...
        type1=symbolTable.symbols[i+j].type.kind;  //形参类型
        type2=T->ptr[0]->type;
        if (type1!=type2){
  ...
  if (T)
	...
	case ID:    //查符号表，获得符号表中的位置，类型送type
                ...
                if (symbolTable.symbols[rtn].flag=='F')
                    ...
                    T->type=symbolTable.symbols[rtn].type.kind;
    ...
    case ArrRef:    //TODO
                ;
                struct node * T1 = T; // Temp Variables
                T->code = NULL;
                int dimension = 0;
                struct opn temp_opn[10];
                int temp_place;
                
                while (T1 != NULL) {
                    if(T1->ptr[0] == NULL && T1->kind != ID) {
                        semantic_error(T1->pos, "", "该类型不可下标引用");
                    }
                    else if (T1->ptr[0] == NULL && T1->kind == ID) {
                        rtn = searchSymbolTable(T1->type_id);
                        if (rtn == -1)  semantic_error(T1->pos, T1->type_id, "数组未定义");
                        if (symbolTable.symbols[rtn].flag=='F')
                            semantic_error(T->pos,T->type_id, "是函数名，类型不匹配");
                        else {
                            if(dimension != symbolTable.symbols[rtn].dimension){
                                semantic_error(T1->pos, T1->type_id, "数组维度不匹配");
                            }

                            T->place = fill_Temp(newTemp(), LEV, INT, 'T', T->offset + T->width);
                            T->width += 4;
                            //result.dimension = dimension;
                            if(dimension==2) {
                                temp_place = fill_Temp(newTemp(), LEV, INT, 'T', T->offset + T->width);
                                opn1 = temp_opn[1];
                                opn2.kind = ID; opn2.offset = symbolTable.symbols[rtn].offset + symbolTable.symbols[rtn].width;
                                strcpy(opn2.id, symbolTable.symbols[temp_place].alias);
                                result.kind = ID; result.offset = T->offset + T->width;
                                strcpy(result.id, symbolTable.symbols[temp_place].alias);
                                T1->code = genIR(STAR, opn1, opn2, result);

                                int temp_place_2 =  fill_Temp(newTemp(), LEV, INT, 'T', T->offset + T->width + 4);
                                opn1 = temp_opn[2];
                                opn2.kind = ID; opn2.offset = symbolTable.symbols[rtn].offset + symbolTable.symbols[rtn].width + 4;
                                strcpy(opn2.id, symbolTable.symbols[temp_place].alias);
                                result.offset = T->offset + T->width + 4;
                                strcpy(result.id, symbolTable.symbols[temp_place_2].alias);
                                T1->code = merge(2, T1->code, genIR(STAR, opn1, opn2, result));

                                opn1.kind = ID; opn1.offset = T->offset + T->width;
                                strcpy(opn1.id, symbolTable.symbols[temp_place].alias);
                                opn2.kind = ID; opn2.offset = T->offset + T->width + 4;
                                strcpy(opn2.id, symbolTable.symbols[temp_place_2].alias);
                                result.kind = ID;
                                result.offset = T->offset + T->width;
                                strcpy(result.id, symbolTable.symbols[T->place].alias);
                                T1->code = merge(2, T1->code, genIR(PLUS, opn1, opn2, result));

                            }else {
                                temp_place = fill_Temp(newTemp(), LEV, INT, 'T', T->offset + T->width);
                                opn1 = temp_opn[1];
                                opn2.kind = ID; opn2.offset = symbolTable.symbols[rtn].offset + symbolTable.symbols[rtn].width;
                                strcpy(opn2.id, symbolTable.symbols[rtn + 1].alias);
                                result.kind = ID; result.offset = T->offset + T->width;
                                strcpy(result.id, symbolTable.symbols[temp_place].alias);
                                T1->code = genIR(STAR, opn1, opn2, result);
                                ...
                                result.offset = T->offset + T->width;
                                strcpy(result.id, symbolTable.symbols[T->place].alias);
                                T1->code = merge(2, T1->code, genIR(ASSIGNOP, opn1, opn2, result));

                            }

                            T1->code = merge(2, T1->code, genIR(ArrRef, opn1, opn2, result));
                    ...
                    if(T1->kind != ID){
                        T1->ptr[1]->offset = T->offset;
                        Exp(T1->ptr[1]);
                        if(T1->ptr[1] != NULL && T1->ptr[1]->kind == FLOAT) {
                            semantic_error(T1->pos, "数组下标不能为负", "");
                        }

                        T->code = merge(2, T->code, T1->ptr[1]->code);
                        T->width += T1->ptr[1]->width;

                        dimension++;
                        temp_opn[dimension].kind = ID;
                        temp_opn[dimension].offset = symbolTable.symbols[T1->ptr[1]->place].offset;
                        strcpy(temp_opn[dimension].id, symbolTable.symbols[T1->ptr[1]->place].alias);
                    }

                    T1 = T1->ptr[0]; // Continue...
    ...
    case FLOAT: T->place=fill_Temp(newTemp(),LEV,T->type,'T',T->offset);   //为浮点常量生成一个临时变量
                ...
                break;
	case ASSIGNOP:  //TODO 
                if (T->ptr[0]->kind!=ID && T->ptr[0]->kind != ArrDec && T->ptr[0]->kind != ArrRef){
                    semantic_error(T->pos,"", "赋值语句需要左值");
                    }
                else if (T->ptr[0]->kind==ID || T->ptr[0]->kind == ArrRef){
                    T->ptr[0]->offset = T->offset;
                ...
                break;
//    case ArrDec:
//                T->ptr[0]->dimension = T->dimension = T->dimension + 1;
//                Exp(T->ptr[0]);
//                Exp(T->ptr[1]);
//                T->place = T->ptr[0]->place;
//                if(symbolTable.symbols[T->place].type.size[symbolTable.symbols[T->place].dimension - T->dimension] < T->type_int) {
...
//                break;
    ...
    case COMPDIV:
    case COMPMUL:
                if (T->ptr[0]->kind!=ID && T->ptr[0]->kind != ArrDec){
                    semantic_error(T->pos,"", "赋值语句需要左值");
                    }
                else if (T->ptr[0]->kind==ID){
                    Exp(T->ptr[0]);   //处理左值，例中仅为变量
    ...
    case PostIncre:
                T->ptr[0]->offset = T->offset;
                Exp(T->ptr[0]);
                result.kind = ID; result.offset = symbolTable.symbols[T->ptr[0]->place].offset;
                strcpy(result.id, symbolTable.symbols[T->ptr[0]->place].alias);
                T->code = genIR(INC, opn1, opn2, result);
                T->code = merge(2, T->ptr[0]->code, T->code);
                /*rtn=searchSymbolTable(T->type_id);
                if (rtn==-1)
                    semantic_error(T->pos,T->type_id, "变量未定义");
                if (symbolTable.symbols[rtn].flag=='F')
  ...
  if (T)
    ...
    case VAR_DEF://处理一个局部变量定义,将第一个孩子(TYPE结点)中的类型送到第二个孩子的类型域
                ...
                while (T0) {  //处理所有DEC_LIST结点
                    ...
                    else if (T0->ptr[0]->kind == ArrDec) { // TODO 处理局部定义的数组
                        struct node* T1 = T0->ptr[0];
                        T1->dimension = 0;
                        num = 1;
                        while (T1->kind != ID){
                            num *= T1->type_int;
                            //T1->offset = T0->offset;
                            //T1->type = T0->type;
                            T1->ptr[0]->dimension = T1->dimension = T1->dimension + 1;
                            T1 = T1->ptr[0];
                        }
                        rtn=fillSymbolTable(T1->type_id,newAlias(),LEV,ARRAY,'V',T->offset);//此处偏移量未计算，暂时为0
                        if (rtn==-1)
                            semantic_error(T1->pos,T1->type_id, "数组重复定义");
                        else {
                            T1->place=rtn;
                            //symbolTable.symbols[rtn].type.kind = ARRAY;
                            symbolTable.symbols[rtn].type.arryKind = T1->type;
                            symbolTable.symbols[rtn].dimension = T1->dimension;
                            T1 = T0->ptr[0];
                            T->width += width * num;
                            T->dimension = T1->dimension;
                            symbolTable.symbols[rtn].width = width * num;
                            int temp = symbolTable.symbols[rtn].dimension;
                            symbolTable.symbols[rtn].type.aux[temp] = 1;
                            while (T1->kind != ID) {
                                symbolTable.symbols[rtn].type.size[temp - T1->dimension] =  T1->type_int;
                                symbolTable.symbols[rtn].type.aux[temp - T1->dimension - 1] = T1->type_int * symbolTable.symbols[rtn].type.aux[temp - T1->dimension];
                                int temp_place =fill_Temp(newTemp(),LEV,T->type,'T',T->offset + T->width); //为整常量生成一个临时变量
                                T->type=INT;
                                opn1.kind=INT;opn1.const_int=symbolTable.symbols[rtn].type.aux[temp];
                                result.kind=ID; strcpy(result.id,symbolTable.symbols[temp_place].alias);
                                result.offset=T->offset + T->width;
                                if (T->dimension == 1)
                                    T->code = genIR(ASSIGNOP,opn1,opn2,result);
                                else 
                                    merge(2, T->code, genIR(ASSIGNOP,opn1,opn2,result));
                                T->width += 4;
                            ...
                            if (rtn==-1)
                                ...
                                opn1.offset = symbolTable.symbols[T0->ptr[0]->ptr[1]->place].offset; //offset
                                result.kind=ID; strcpy(result.id,symbolTable.symbols[T0->ptr[0]->place].alias);
                                result.offset = symbolTable.symbols[T0->ptr[0]->place].offset; //offset
...
//                        T->width+=T->ptr[1]->width;//顺序结构顺序分配单元方式
	...
	case RETURN:if (T->ptr[0]){
                    ...
                    if (T->ptr[0]->type!=symbolTable.symbols[num].type.kind) {
