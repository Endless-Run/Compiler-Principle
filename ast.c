...
void display(struct node *T,int indent)  {//对抽象语法树的先根遍历
  ...
  if (T)
	...
	case TYPE:          printf("%*c类型： %s\n",indent,' ',T->type_id);
                        break;
	case StructDec:      printf("%*c结构体声明： \n",indent,' ');
						display(T->ptr[0],indent+3);
						break;
	case StructDef: 	display(T->ptr[0],indent);
						break;
	case StructSpecifier:	printf("%*c结构体： \n",indent,' ');
							display(T->ptr[0],indent+3);
    ...
    case FOR:           printf("%*cfor循环语句：\n",indent,' ');
                        ...
                        display(T->ptr[1],indent+6);      //显示循环体
                        break;                    
    case ForExp:        display(T->ptr[0],indent);   
    					display(T->ptr[1],indent);
    					display(T->ptr[2],indent);
    					break;
	case PreIncre:		printf("%*c前自增: %s\n", indent, ' ', T->type_id);
						break;			
	case PreDecre:		printf("%*c前自减: %s\n", indent, ' ', T->type_id);
						break;						
	case PostIncre:		printf("%*c后自增: %s\n", indent, ' ', T->type_id);
						break;						
	case PostDecre:		printf("%*c后自减: %s\n", indent, ' ', T->type_id);
						break;						
    ...
    case VAR_DEF:       printf("%*cLOCAL VAR_NAME：\n",indent,' ');
                        ...
                        break;
	case ArrDef:		if(T->ptr[0]->kind != ID) {
                            printf("%*cArray Definition: \n", indent, ' ');//数组 
                            display(T->ptr[0], indent + 3);//显示数组名 
                        }
                        else {
                            display(T->ptr[0], indent);//显示变量名
                        }
						break;
    case ArrDec:
                        display(T->ptr[0], indent);
						printf("%*cLength: %d\n", indent, ' ', T->type_int);//显示数组长度 
                        break;
    case DEC_LIST:      
                        display(T->ptr[0], indent);
                        display(T->ptr[1], indent);
